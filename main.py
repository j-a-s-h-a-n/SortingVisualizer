import pygame
import random
from infoClass import DrawInformation
pygame.init()
...
def draw(draw_info, algo_name, ascending):
    ...
    speeds = draw_info.FONT.render("Down - Decrease Speed | Up - Increase Speed", 1,draw_info.BLACK)
    draw_info.window.blit(speeds, (draw_info.width / 2 - speeds.get_width() / 2, 105))
...
def selection_sort(draw_info, ascending=True):
    ...
    for ind in range(size):
        ...
        draw_list(draw_info, {min_index: draw_info.GREEN, ind: draw_info.RED}, True)
        yield True
    return lst
...
def main():
    ...
    while run:
        if sorting_algo_name=="Selection Sort":
            clock.tick(10*Speed)
        else:
            clock.tick(60*Speed)
        if sorting:
